> **SUPERSEDED, 2026-08-21.** This describes a system the site no longer runs. It sets
> Inter and Instrument as the typefaces and a deep lime `#5B3A59` as the accent. The site
> now runs M PLUS Rounded 1c and DM Mono on a blue grey ground with a sky blue accent
> `#2E9BE6`. Kept because it carries ratified council decisions and the reasoning behind
> them. **Do not work from this file.** The live guide is
> `docs/04_DESIGN_SYSTEM/STYLE_GUIDE.md` and `src/styles/index.css` beats both.
